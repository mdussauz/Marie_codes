function [CR_boot,TuningCurve, XBins, PairedCorrs, PairedResiduals, ControlCorrs, ControlResiduals] = ...
    GetOdorTuningCurves_v2(SessionPath,MyUnits,varargin)
...
for boot = 1:10
    for whichodor = 1:3
        ...
        TuningCurve.ClosedLoopHalf1(:,:,:,whichodor,boot) = squeeze(myCurve(:,[1 3],:,:));
        [myCurve, ~] = SmellocatorTuning(TuningBinType,125-XVar(find(~TossVar),:),1000*(YVar(find(~TossVar),:)/Binsize),customBins);
        TuningCurve.ClosedLoopHalf2(:,:,:,whichodor,boot) = squeeze(myCurve(:,[1 3],:,:));
    end
end
...
TemplateTrials = find(strcmp(TrialInfo.Perturbation,'OL-Template'));
    ...
    for i_boot = 1:10
        [PairedCorrs,PairedResiduals,ControlCorrs,ControlResidualsboot] = CompareTuningCurves_v2(TuningCurve,i_boot);
        CR_boot(:,i_boot) = ControlResidualsboot(:,1);
    end
    mean_CR_all_boot = mean(CR_boot,2);
    ControlResiduals = [];
    ControlResiduals = vertcat(ControlResiduals, [mean_CR_all_boot ControlResidualsboot(:,2) ControlResidualsboot(:,3)]);
end
