function [spikeTrace,fullV,c,cFull] = TestRidgeOnOpenLoopSession02142022()
%% what session
...
fullR = [Odor1Location Odor2Location Odor3Location interpolated_thermistor];
...
regLabels = {'Odor1 location' 'Odor2 location' 'Odor3 location' 'Sniff'};
...
regIdx = [
    ...
    ones(1,size(interpolated_thermistor,2))*find(ismember(regLabels,'Sniff'))...
    ];
...
regGroups = {'Odor1 location' 'Odor2 location' 'Odor3 location' 'Sniff'};
...
plot(cRegs(c, 1, SniffReg), 'linewidth', 2);
%plot(cRegs(c, 1, TrialReg), 'linewidth', 2);
...
legend('full model','Odor1', 'Odor2', 'Odor3', 'Sniff')

subplot(1,2,2)
fullMean = [
            mean(cRegs(:, 1, Odor1Reg)) ...
            mean(cRegs(:, 1, Odor2Reg)) ...
            mean(cRegs(:, 1, Odor3Reg)) ...
            mean(cRegs(:, 1, SniffReg)) ...
            ];
           %mean(cRegs(:, 1, TrialReg)) 
           %mean(cRegs(:, 1, LeverVelReg)) 
           %mean(cRegs(:, 1, LeverPosReg))
fullError = [
            sem(cRegs(:, 1, Odor1Reg)) ...
            sem(cRegs(:, 1, Odor2Reg)) ...
            sem(cRegs(:, 1, Odor3Reg)) ...
            sem(cRegs(:, 1, SniffReg)) ...
            ];
            %sem(cRegs(:, 1, TrialReg))
...
uniqueMean = [
                mean(cFull-cRegs(:, 2, Odor1Reg))...
                mean(cFull-cRegs(:, 2, Odor2Reg))... 
                mean(cFull-cRegs(:, 2, Odor3Reg))...
                mean(cFull-cRegs(:, 2, SniffReg))...
                ];
              %mean(cFull-cRegs(:, 2, TrialReg)) 
              %mean(cFull-cRegs(:, 2, LeverVelReg))
              %mean(cFull-cRegs(:, 2, LeverPosReg))
uniqueError = [
                sem(cFull-cRegs(:, 2, Odor1Reg))...
                sem(cFull-cRegs(:, 2, Odor2Reg))...
                sem(cFull-cRegs(:, 2, Odor3Reg))...
                sem(cFull-cRegs(:, 2, SniffReg))...
                ];
               %sem(cFull-cRegs(:, 2, TrialReg)) 
...
set(ax,'xTickLabel',{'Odor1', 'Odor2', 'Odor3', 'Sniff'})
