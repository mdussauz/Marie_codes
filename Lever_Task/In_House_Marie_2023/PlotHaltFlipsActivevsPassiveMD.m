function [AlignedFRs, BinOffset, whichTZ] = PlotHaltFlipsMD(whichUnit, whichOdor, AlignedSpikes, Events, TrialInfo, AlignTo, AAlignedSpikes, AEvents, ATrialInfo)

% 
whichodor = whichOdor;

% get halt flip trials
perturbationTrials = intersect(find(strcmp(TrialInfo.Perturbation(:,1),'Halt-Flip-Template')), ...
...
whichTrials = intersect(find(strcmp(ATrialInfo.Perturbation(:,1),'Halt-Flip-Template')), ...
    find(ATrialInfo.Odor==whichodor));
whichTrials = intersect(whichTrials, find(ATrialInfo.TargetZoneType==whichTZ));
whichTrials = [whichTrials ATrialInfo.TargetZoneType(whichTrials) ATrialInfo.Duration(whichTrials)]; %#ok<AGROW>
...
PthisUnitSpikes = AlignedSpikes(:,whichUnit);
thisUnitSpikes = AAlignedSpikes(:,whichUnit);
whichodor = whichOdor;
...
if plotting
    ...
    for x = 1:size(whichTrials,1)
        ...
        ZoneTimes = ATrialInfo.InZone{whichTrials(x)} - Offset(x);
...
if ~isempty(perturbationTrials)
    if plotting
        ...
        for y = 1:size(perturbationTrials,1)
            ...
            % Plot Spikes
            thisTrialSpikes = PthisUnitSpikes{perturbationTrials(y,1)}{1};
            % adjust spiketimes if needed
...
% calculate PSTH
for TZ = whichTZ
    thisTZspikes = PthisUnitSpikes(perturbationTrials(find(perturbationTrials(:,2)==TZ),1));
