function [] = PlotRespondTrend_v2(smoothPSTH)
...
%% calculate z score
mu = squeeze(mean(firingRatesAverage(:,:,:,1:6000),[2 3 4]));
sigma = squeeze(std(firingRatesAverage(:,:,:,2:6000),0,[2 3 4]));
z_score = (firingRatesAverage(:,:,:,:) - mu) ./ sigma;
z_score_matrix(:,:,:) = mean(z_score(:,:,:,6000:8000),4);
...
for whatsmell = 1:5
    ...
    for whatconc = 1:4
        ...
        airmean = squeeze(nanmean(firingRatesAverage(:,:,:,1:6000),[1 2 3 4]));
        odormean = squeeze(nanmean(firingRatesAverage(:,whatsmell,whatconc,6000:8000),[1 4]));
        ...
        %Excitatory
        airmeanExc = squeeze(nanmean(excited_units(:,:,:,1:6000),[1 2 3 4]));
        odormeanExc = squeeze(nanmean(excited_units(:,whatsmell,whatconc,6000:8000),[1 4]));
        %Inhibitory 
        airmeanInh = squeeze(nanmean(inhibited_units(:,:,:,1:6000),[1 2 3 4]));
        odormeanInh = squeeze(nanmean(inhibited_units(:,whatsmell,whatconc,6000:8000),[1 4]));
