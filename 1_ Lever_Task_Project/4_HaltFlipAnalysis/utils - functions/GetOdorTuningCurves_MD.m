...
%TuningCurve will contain mean, median, std and samples for each location
%in dim 2 of the TuningCurve matrix 

%TuningCurve = 13 locations x (mean, median, std) x nb units x 3 odors
...
for whichodor = 1:3
    ...
    [myCurve, XBins] = SmellocatorTuning(TuningBinType,125-XVar,1000*(YVar/Binsize),customBins); % myCurve = [mean, median, std, numel]
    TuningCurve.ClosedLoopFull(:,:,:,whichodor) = squeeze(myCurve(:,[1 2 3 4],:,:));
    [myCurve, ~] = SmellocatorTuning(TuningBinType,125-XVar(find(TossVar),:),1000*(YVar(find(TossVar),:)/Binsize),customBins);
    TuningCurve.ClosedLoopHalf1(:,:,:,whichodor) = squeeze(myCurve(:,[1 2 3 4],:,:));
    [myCurve, ~] = SmellocatorTuning(TuningBinType,125-XVar(find(~TossVar),:),1000*(YVar(find(~TossVar),:)/Binsize),customBins);
    TuningCurve.ClosedLoopHalf2(:,:,:,whichodor) = squeeze(myCurve(:,[1 2 3 4],:,:));
end
