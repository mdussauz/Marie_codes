function [WFcorrelation, concWFcorrelation, CorrelationCriteria, concCorrelationCriteria] = GetWaveformCorrelation (SessionName, sessionHalfTime)
%input:
...
for mycluster = 1:length(sp.cids) % for each cluster
    ...
    concWFcorrelation(mycluster) = corr(conc_meanWF_first', conc_meanWF_second','Type', 'Pearson');
